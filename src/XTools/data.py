...
def get_list_value(now_list: list, target_key, results=[], mod='value'):
    ...
    for i in now_list:  # 当前迭代的list
        if isinstance(i, dict):  # 如果data是一个字典，就递归遍历
...
def get_dict_value(now_dict: dict, target_key, results=[]):
    ...
    for key, value in now_dict.items():  # 当前迭代的字典
        if isinstance(value, dict):  # 如果data是一个字典，就递归遍历
...
def wr_first(cfg, write_path, dict1, k=None, n=0, mod='change'):
    """
    ...
    """
    # sourcery skip: default-mutable-arg
    write_path = op.abspath(write_path)
    if isinstance(k, str):
        k = key_clever(k)
